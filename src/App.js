...
const tld = '.digital';
const CONTRACT_ADDRESS = '0x41f53d4e31073E3bb42F6D5d528eA0B98e758826';
...
const App = () => {
		...
		const [network, setNetwork] = useState(''); // State Networks
		const [domain, setDomain] = useState(''); // Add some state data properties
  	const [record, setRecord] = useState('');
  	const [editing, setEditing] = useState(false); // Editing Record Stateful Variable
  	const [mints, setMints] = useState([]); // Stateful record for minting.
  	const [loading, setLoading] = useState('')
		...
		const chainId = await ethereum.request({ method: 'eth_chainId' });
			setNetwork(networks[chainId]);      // Checking the users network chain ID
	
			ethereum.on('chainChanged', handleChainChanged);
	
			function handleChainChanged(_chainId) {    // Reload the page when they change networks
	...
	const mintDomain = async () => {
		if (!domain) { return }				// Don't run if the domain is empty
		if (domain.length < 3) {			// Alert the user if the domain is too short
	...
	useEffect(() => { 				// This will run any time currentAccount or network are changed
  ...
  const renderMints = () => {
	...
	const renderInputForm = () =>{
		...
		if (network !== 'Polygon Mumbai Testnet') {  			// If not on Polygon Mumbai Testnet, render "Please connect to Polygon Mumbai Testnet"
				return (
					<div className="connect-wallet-container">
